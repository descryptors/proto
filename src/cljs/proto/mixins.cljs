...
(defn window-event-listener
  [& [{:keys [on-mount on-update]}]]
  (let [listeners (atom nil)]
    (r/create-class
     {:display-name "proto.mixins.window_event_listener_class"
      :component-did-mount (fn [this]
                             (when on-mount (on-mount this))
                             (update-listeners js/window listeners (r/props this) this))
      :component-did-update (fn [this]
                              (when on-update (on-update this))
                              (update-listeners js/window listeners (r/props this) this))
